...
CommandsChain* parseSimpleCommand(string& line, unsigned int& begin)
{
    ...
    while (length > 0)
    {
        ...
        if (s == ">" || s == ">>" || s == "<")
        {
            begin = getNextTokenPosition(line, begin + length);
            length = getTokenLength(line, begin);
            string f = line.substr(begin, length);

            if (length == 0)
            {
                cout << "Error: Expected file name at position "
                    << begin << endl;
                return NULL;
            }
            else if (f == ")" || f == "]" || f == ";" || f == "||" || f == "&&"
            ...
            else
            {
                if (s == ">")
                {
                    if (!command->setOutputRedirection(RM_SIMPLE, f))
                    {
                        cout << "Error: Only one output redirection per command is allowed. At position "
                            << begin << endl;
                        return NULL;
                    }
                }
                ...
                {
                    if (!command->setOutputRedirection(RM_APPEND, f))
                    {
                        cout << "Error: Only one output redirection per command is allowed. At position "
                            << begin << endl;
                        return NULL;
                ...
                {
                    if (!command->setInputRedirection(RM_SIMPLE, f))
                    {
                        cout << "Error: Only one input redirection per command is allowed. At position "
                            << begin << endl;
                        return NULL;
                    }
                }
        ...
        string precedingConnector)
{
    ...
    if (connector == "|")
    {
        if (((Command*)chain) == NULL)
        {
            cout << "Error: Redirection can be applied only to 2 ordinary commands. At position "
                << begin << endl;
            delete chain;
            return NULL;
        }
        if (!((Command*)chain)->setOutputRedirection(RM_PIPE, ""))
        {
            cout << "Error: Only one output redirection per command is allowed. At position "
                << begin << endl;
            delete chain;
    ...
    if (connector == "|")
    {
        ...
        if (!((Command*)next)->setInputRedirection(RM_PIPE, ""))
        {
            cout << "Error: Only one input redirection per command is allowed. At position "
                << begin << endl;
            delete next;
